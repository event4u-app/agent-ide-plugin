...
- [~] **T-PRD05 — permission cards, full (#5).** <!-- core half DONE 2026-05-31 (feat/product-readiness-trust-control, ADR-014): `permissions/audit.ts` AuditLog — append-only date-rotated JSONL audit trail (grant_once/grant_always/deny_user/deny_hard_floor, fail-open, torn-line tolerant), wired into `runToolCallWithApproval` via an optional injected `audit` recorder (records decisions + hard-floor blocks, NOT auto-allows); `classifyRisk(level)` → low|medium|high derived risk badge (never persisted, UI hint not a boundary). 11 unit tests. AI council UNANIMOUS (separate log over permissions.json bloat). Card RENDER + audit-trail link UI stay IDE-runtime → `[~]`. RISK-BADGE WIRE CONTRACT now DONE 2026-06-01 (feat/product-readiness-approval-risk-level, ADR-032): `classifyRisk` (shipped uncalled in ADR-014) is now wired onto the `approvalRequested` event — protocol gains `RiskLevelSchema` + a required `riskLevel` field, `runToolCallWithApproval` computes `classifyRisk(verdict.level)` and emits it on the event only (fork B2: presentation hint, the decider keeps the authoritative `level`); core owns the classification so both clients render one consistent badge. AI council UNANIMOUS A1/C1/D2, SPLIT B→B2. Codegen → `Protocol.kt val riskLevel: String`. +2 core tests incl. a protocol↔core enum drift guard; updated protocol fixtures for the now-required field (core 990/1 skip, protocol 41). Only the badge CHIP render stays IDE → STILL `[~]`. AUDIT-TRAIL NOW LIVE 2026-06-02 (feat/product-readiness-audit-log-wiring, ADR-038): the AuditLog + its four `runToolCallWithApproval` call sites shipped in ADR-014, but `buildCoreDispatcher` (the composition root) NEVER constructed a recorder → `deps.audit` was always `undefined` → every `record()` was a no-op → the production trail was DEAD. Now `buildCoreDispatcher` constructs ONE `AuditLog` under `<state>/audit` (date-rotated `audit-<YYYY-MM-DD>.jsonl`, its own sibling of cost/tracking — council Q1=A) and injects it into the agent turn ONLY (chat has no approval path — Q2=A); `BuildCoreOptions.audit?` override mirrors the step/calibration/budget pattern (Q3=A). With the deny-default `decide` the trail is live the moment a tool is gated (every denied write → `deny_user`, every hard-floor block → `deny_hard_floor`). NO protocol/codegen change. AI council (codex 0.134.0 + gemini 0.41.2) UNANIMOUS Q0/Q1/Q2/Q3/Q5, SPLIT Q4→A (leave the competing dead `tracking/audit-log.ts` T-413 untouched, minimal-diff — flagged for a follow-up cleanup; gemini's confusion concern documented in the ADR not deleted). Known limitation (gemini trap): the entry schema carries no session/conversation correlation — a separate schema-evolution slice. +1 core test (sidecar.test.ts: default-constructed recorder writes a `deny_user` row to disk on a denied write_files; core 1036/1 skip). The card's audit-trail LINK + the drawer UI stay IDE-runtime → STILL `[~]`. --> Diff preview, risk-level badge (from the hard-floor classifier), `allow once / allow always / deny`, and a link to the audit-trail entry. Always-rules persist per workspace.
