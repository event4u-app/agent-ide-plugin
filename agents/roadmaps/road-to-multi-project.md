...
- [ ] An ADR records the `WorkspaceRoot` identity model (`uri` / `stableId` / `canonicalKey`) and the nested-root / dedup rule (created via the `adr-create` flow).
