...
- [~] **T-103 — JetBrains plugin skeleton.** `clients/jetbrains/` with `build.gradle.kts` using `intellij-platform-gradle-plugin v2`. `plugin.xml` declares ToolWindow `event4u-agent` right-anchored, depends on `com.intellij.modules.platform`. Add `AgentToolWindowFactory.kt` returning a placeholder JPanel. Verify plugin installs in PhpStorm 2024.2+ via `task jetbrains:runIde`. <!-- build.gradle.kts (IntelliJ Platform Gradle Plugin 2.2.1, sinceBuild 242, JVM17, detekt+ktlint), plugin.xml, AgentToolWindowFactory.kt (Swing placeholder), SidecarClient.kt. COMPILES + passes detekt + ktlint green on JDK-17 CI (PR #2, gradle check). Still [~]: `runIde` plugin-install verification needs a human-driven IDE (no GUI in CI). -->
