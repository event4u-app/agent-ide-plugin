...
- [~] **T-106 — GitHub Actions CI.** `.github/workflows/ci.yml` with matrix: ubuntu/macos/windows × node 20/22. Steps: `pnpm install`, lint, format, typecheck, build, test. Kotlin job runs `./gradlew check`. Total CI time target: < 10 min. <!-- authored: ci.yml with node 3-OS×2-version matrix (exact steps verified green locally) + a jetbrains gradle-check job marked continue-on-error (Kotlin not JVM-verified locally). Flips to [x] once CI runs green on GitHub (next: PR + fix-CI). -->
